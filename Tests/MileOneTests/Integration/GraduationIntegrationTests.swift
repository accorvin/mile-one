...
@MainActor
@Suite("Graduation Integration Tests")
struct GraduationIntegrationTests {
    ...
    private func seedProfile(in dataStore: MileOne.DataStore, week: Int, sessions: Int, graduated: Bool = false) async throws {
    ...
    func advanceWeekAt9SetsGraduated() async throws {
        ...
        #expect(profile?.hasGraduated == true, "hasGraduated must be set to true after completing week 9")
        #expect(profile?.currentWeek == 9, "currentWeek should stay at 9 after graduation")
    }
    ...
    func advanceWeekAt9WithoutSessionsDoesNotGraduate() async throws {
        ...
        let profile = try await dataStore.fetchUserProfile()
        #expect(profile?.hasGraduated == false, "hasGraduated must not be set without 3 completed sessions")
    }
    ...
    func mockAdvanceWeekAt9SetsGraduated() async throws {
        ...
        #expect(profile?.hasGraduated == true, "Mock must also set hasGraduated on week 9 completion")
        #expect(profile?.currentWeek == 9)
    }
    ...
    @Test("Completing week 8 does not set hasGraduated")
    func completingWeek8DoesNotGraduate() async throws {
        ...
        let profile = try await dataStore.fetchUserProfile()
        #expect(profile?.hasGraduated == false, "Graduation only triggers at week 9, not week 8")
        #expect(profile?.currentWeek == 9)
    }
}
